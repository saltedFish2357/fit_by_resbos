...
std::vector<exp_parameter> get_exp_Afb_list(ROOT::RDataFrame exp_df, bin Rapidity_bin, bin Qt_bin, std::vector<bin> Mass_bin_list)
{
    ...
    auto filtered_exp_df = exp_df
                               .Define("Rapidity", [](const double &Z_Px, const double &Z_Py, const double &Z_Pz, const double &Z_E)
                                        {
                                            return MyAnalysis::Utils::get_Rapidity(Z_Px, Z_Py, Z_Pz, Z_E);
                                        }, {"ZPx", "ZPy", "ZPz", "ZE"})
                               .Filter([Rapidity_bin](const double &Rapidity) { return Rapidity_bin.is_in_bin(std::abs(Rapidity)); },
                               ...
                               .Filter([Qt_bin](const double &Qt) { return Qt_bin.is_in_bin(Qt); },
                                       {"Qt"}, "Qt_cut")
                               ...
                               .Filter([Mass_bin_list](const double &Z_M)
                                       ...
                                       {"Z_M"}, "Z_M_cut")
                               .Define("cos_Theta_h", [](const double &Z_Pz, const double &Z_M, const double &Qt, 
                                                           const double &Lepton_Pz, const double &AntiLepton_Pz, 
                                                           const double &Lepton_E, const double &AntiLepton_E)
                                        {
                                            return MyAnalysis::Utils::get_cos_Theta_h(Z_Pz, Z_M, Lepton_Pz, Qt, 
                                                                                     AntiLepton_Pz, Lepton_E, AntiLepton_E);
                                        }, {"ZPz", "Z_M", "Qt", "LeptonPz", "AntiLeptonPz", "LeptonE", "AntiLeptonE"});
    ...
    auto forward_hist_df = filtered_exp_df
                           .Filter([](double cos_Theta_h) { return cos_Theta_h > 0; },
                                   {"cos_Theta_h"}, "forward_cut")
                           .Histo1D({"exp_forward_hist", "exp_sigma_forward;Z_Mass(GeV);Counts", 
                                    Mass_bin_list.size(), mass_edges.data()}, "Z_M");
    ...
    // 获取直方图指针并转移所有权到智能指针
    forward_hist.reset(forward_hist_df.GetPtr()->Clone("forward_hist_clone"));
    backward_hist.reset(backward_hist_df.GetPtr()->Clone("backward_hist_clone"));
